...
export function createPolicy(cfg: ToolPolicyConfig): ToolPolicy {
  ...
  function check(name: string, args: Record<string, unknown>): PolicyDecision {
    ...
    if (meta.kind === 'exec') {
      ...
      const writeLike = /(^|[;&|]\s*)(rm|mv|cp)\b|>\s*[^&|;]*\.agents\/(?:skills|workflows|business-plugins|runforge-workload-sdk)|tee\b[^&|;]*\.agents\/(?:skills|workflows|business-plugins|runforge-workload-sdk)|sed\s+-i\b/.test(command);
