...
const [source, destination, language = 'zh'] = process.argv.slice(2);
if (!source || !destination) throw new Error('用法: transcribe.mjs <音频路径> <输出.srt> [语言]');
...
const baseUrl = (await client.secrets.get('audio-video-tools.speaches-url')).replace(/\/+$/, '');
const key = await client.secrets.get('audio-video-tools.speaches-key');
...
if (!srt.trim()) console.log('转写为空：没有识别到有效讲话');
