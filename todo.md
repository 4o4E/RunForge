...
### 11. 引入 subagent 并行处理能力

现状：

- 当前一个 run 主要由单个 agent 循环串行推进。
- 还没有 subagent 任务拆分、并发执行、结果汇总和失败隔离机制。
- 受限于时间，这部分尚未开始实现。

要做：

- 增加 subagent 调度接口，让主 agent 可以把独立子任务拆给多个 subagent 并行处理。
- 为 subagent 建立独立上下文、工具权限、事件流和结果回收协议。
- 支持主 agent 汇总多个 subagent 的输出，并处理部分失败、超时和取消。
- 在前端区分主 agent 与 subagent 的执行过程，避免用户看不清任务来源。

为什么：

- 大型代码阅读、资料检索、批量验证这类任务天然可并行。subagent 能缩短耗时，并减少单一上下文过大的压力。

### 12. 增加常驻 shell 支持长耗时任务

现状：

- 当前 shell 工具更偏一次性命令执行。
- 长耗时任务需要反复启动命令，无法稳定保留交互式进程、后台日志和运行状态。
- 受限于时间，这部分尚未开始实现。

要做：

- 为每个 run 或 workspace 提供可复用的常驻 shell 会话。
- 支持启动、轮询、写入 stdin、读取增量输出、终止和超时回收。
- 将常驻 shell 的进程状态、最近输出和退出码写入 event，便于前端展示。
- 和沙箱、资源限制、取消 run 打通，避免长进程失控。

为什么：

- 构建、测试、爬取、模型评测等任务可能运行很久。常驻 shell 可以让 agent 持续观察进度，而不是阻塞在单次工具调用里。

### 13. 建立 skill 系统沉淀业务经验和流程

现状：

- 当前系统提示词和工具是通用能力。
- 还没有把特定业务经验、项目流程、验收规范沉淀成可复用 skill 的机制。
- 受限于时间，这部分尚未开始实现。

要做：

- 定义 skill 元数据、触发条件、适用边界和加载顺序。
- 支持按任务类型加载对应流程文档、检查清单和示例。
- 让 skill 可以引用仓库内文档、脚本或模板，减少重复提示词。
- 在事件或调试信息中展示本次 run 命中了哪些 skill。

为什么：

- skill 能把“怎么做这类任务”的经验文档化，减少 agent 每次从零推理，也让复杂流程更稳定、可复查。

### 14. 建立 memory 系统积累跨对话经验
