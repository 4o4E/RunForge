...
- 外部空间在 Web 中始终只读。对话页保留完整输入区域和运行状态展示，包括实际运行模型、
  上下文占用、Plan、进度和实时流式事件；输入框、模型选择、附件、发送、继续、回答、取消、
  分支和修改等会改变会话状态的交互全部禁用。Web 用户不能创建、追加、继续、回答、取消、
  分支或修改外部任务；这些限制必须由 API 强制，而不是只隐藏按钮。
...
- 长期 secret 的唯一隔离维度是 `tenant + key -> 当前唯一值`，不增加 user、space、run
  维度，也不保存历史版本。
